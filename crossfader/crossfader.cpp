...
void CrossFader::run()
{
    ...
    for (int x = 0; x < image.width(); ++x) { // Naive and slow!
        for (int y = 0; y < image.height(); ++y) {
            QRgb firstPixel = m_first.pixel(x, y);//pixel操作非常慢
        ...
        if (m_stopped)
            return;
        emit progress(qRound(x / onePercent));//完成后发射progress信号到主线程
    }
    ...
    for (int y = 0; y < image.height(); ++y) { // Faster
        QRgb *firstPixels = reinterpret_cast<QRgb*>(
                m_first.scanLine(y));//scanLine把像素数据作为单一连续数组存储起来
    ...
    const int onePercent = qRound(image.width() * image.height() /
                                  100.0);
    QRgb *firstPixels = reinterpret_cast<QRgb*>(m_first.bits());//bits
