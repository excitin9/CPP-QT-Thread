...
{
	threadsafe_queue<int> tq;//1 创建空队列
	std::promise<void> push_ready;//2 为准备状态创建promise对象
	std::promise<void> pop_ready;
	std::promise<void> go;
	std::shared_future<void> ready(go.get_future());//3 为go信号获取一个std::shared_future对象
	std::future<void> push_done;//4 创建了future用来表示线程是否结束
	std::future<int> pop_done;

	try
	{
		push_done = std::async(std::launch::async,
			[&tq, ready, &push_ready]()//5 
			//注意，使用 std::async 会让你任务更容易成为线程安全的任务
		{
		...
		go.set_value(); //9 而主线程不同——等待所有线程的信号前,提示所有线程进行测试
		push_done.get();//10 异步调用等待线程完成 
		//assert(pop_done.get() == 43); //11
		//assert(tq.empty());
	}
