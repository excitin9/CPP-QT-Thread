...
struct sorter //1
{
	...
	std::list<T> do_sort(std::list<T> &chunk_data)
	{
		...
		typename std::list<T>::iterator divide_it = std::partition(chunk_data.begin(), chunk_data.end(),
			[&](const T &val) { return val < partition_val; }); //返回分割的位置

		std::list<T> new_lower_chunk; //值较小的一边
		new_lower_chunk.splice(new_lower_chunk.end(), chunk_data, chunk_data.begin(), divide_it);
		std::future<std::list<T>> new_lower = //3 绑定this指针到do_sort()上，并传入std::move(new_lower_chunk)参数
			m_tp.submit(std::bind(&sorter::do_sort, this, std::move(new_lower_chunk))); //数据移动要比拷贝的方式开销少
		...
		{
			m_tp.run_pending_task();//4 执行任务队列上未完成的任务
		}
...
//int main()
//{
//	std::list<int> ln{ 24, 34, 324, 23, 4, 24, 2, 4, 2, 4, 25, 3, 5, 546, 8, 67, 
//		8, 78, 78, 980, 4, 345, 7, 79, 765, 62, 34, 2346, 0, 98, 77, 32, 34, 89, 
//		67, 85, 45, 234, 4, 6, 8, 6, 54, 3, 7, 87, 9, 65, 45, 532, 4, 3, 4 };
//
