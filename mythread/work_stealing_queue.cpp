...
public:
	...
	void push(function_wrapper data) //2
	{
	...
	bool try_pop(function_wrapper &value) //3
	{
		std::lock_guard<std::mutex> lk(m_mutex);
		if (m_deuqe.empty())
		{
	...
	bool try_steal(function_wrapper &value)//4
	{
		std::lock_guard<std::mutex> lk(m_mutex);
		if (m_deuqe.empty())
		{
...
private:
	std::deque<function_wrapper> m_deuqe; //1
...
public:
		...
		: m_done(false), m_joiner(m_threads)
	{
		...
		try
		{
			...
			{
				//每个线程被创建，就创建了一个属于自己的工作队列
				m_queues.push_back(std::make_unique<work_stealing_queue>()); //6 
			}
			for (unsigned index = 0; index < thread_count; ++index)
			{
	...
	void run_pending_task()
	{
		...
		if (pop_task_from_local_queue(task) //7 从线程队列中取
			|| pop_task_from_pool_queue(task) //8 从线程池中取
			|| pop_task_from_other_thread_queue(task)) //9 从其他线程的队列中获取
		{
			task();
		}
		else
...
private:
	...
	//增加这条，为每个线程都生成一个worl_stealing_queue
	std::vector<std::unique_ptr<work_stealing_queue>> m_queues; //1 
	std::vector<std::thread> m_threads;
	join_threads m_joiner;
	static thread_local work_stealing_queue* sm_local_work_queue;//2
	static thread_local unsigned sm_my_index;

	void work_thread(unsigned my_index)
	{
		sm_my_index = my_index;
		//列表中队列的序号，会传递给线程函数，然后使用序号来索引对应队列
		sm_local_work_queue = m_queues[my_index].get();//3
		while (!m_done)
		{
			run_pending_task();
		}
