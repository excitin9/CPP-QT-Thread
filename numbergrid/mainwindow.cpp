...
QString stringForLongDouble(const long double &x)
{
    ...
    int i = snprintf(longDouble, BUFFER_SIZE, "%.3Lf", x);//#include <cstdio>
...
public:
    ...
    typedef SurrogateItem result_type;//必须提供result_type typedef才能被传递给QtConcurrent函数

    //类的转换函数： 可以将SurrogateItemApplier转换为SurrogateItem
...
void MainWindow::editSelect()
{
    ...
    if (matchForm.exec()) {
        ...
        //QtConcurrent::filtered(要处理的项的序列，过滤函数) （functor函数）
...
void MainWindow::editApplyScript()
{
    ...
    if (scriptForm.exec()) {
        script = scriptForm.script();//脚本对话框允许用户输入脚本
        stop();
        view->setEditTriggers(QAbstractItemView::NoEditTriggers);
        errorInfo.clear();
        applyToAll = scriptForm.applyToAll();
        //如果applyToAll就可以应用到全部单元格上
        QList<SurrogateItem> items = applyToAll ? allSurrogateItems()
                                           : selectedSurrogateItems();
        //mapped带一个 项序列、一个映射函数或仿函数
...
bool MainWindow::applyDespiteErrors()
{
    ...
    if (errors.count() > MaxErrorStrings) {
        ...
        //如果发生了一个或多个错误，会弹出一个对话框，显示不超过15个错误信息
        //如果多于15个，则要提示还剩下多少个
...
void MainWindow::stop()
{
    ...
    //调用QFutureWatcher<T>::waitForFinished() 用来阻塞线程，确保线程真正停止
    if (selectWatcher.isRunning())
